...
bool VectorPursuitController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
        ...
        isCollisionImminent(robot_pose, linear_vel, angular_vel, target_dist))
    {
        ...
        ROS_ERROR("%s: Collision detected ahead!", controller_name_.c_str());
        return false;
    }
    ...
    nav_msgs::Path& transformed_plan)
{
    ...
    if (transformed_plan.poses.empty()) {
        ROS_ERROR("%s: Resulting plan has 0 poses in it.", controller_name_.c_str());
        return false;
    }
    return true;
}
    ...
    const double & theta)
{
    ...
    {
        ROS_WARN("%s: Footprint cost is unknown, collision check failed", controller_name_.c_str());
        return false;
    }
